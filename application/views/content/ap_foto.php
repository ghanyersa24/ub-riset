					...
					<div class="row">
						<div class="col-md-4">
							...
							<div class="d-flex justify-content-center w-100 ">
								<img style="display: none; max-height:300px; max-width:75%" src="" alt="foto produk" id="prev-add-foto" class="text-center">
							</div>
						</div>
						<div class=" col-md-8">
							<div class="form-group">
				...
				<div class="modal-body" id="form-data">
					<div class="row">
						<div class="col-md-4">
							...
							<div class="d-flex justify-content-center w-100 ">
								<img src="" alt="foto produk" id="prev-view-foto" class="text-center" style="max-height:300px; max-width:100%">
							</div>
						</div>
						<div class="col-md-8">
							<div class="form-group">
...
<script>
	...
	function get() {
		$.ajax({
			type: "GET",
			url: api + 'service/foto_produk/get/<?= $id ?>',
			success: function(response) {
				res = response.data
				let card = ""
				if (res.length == 0)
					card = `<div class="col-12 text-center"> <p class="h5">silahkan tambahkan foto produk anda</p></div>`
				else
					res.forEach(element => {
	...
	function del(id, link) {
		swal({
				...
				text: "menghapus foto produk ini secara permanen!",
				icon: "warning",
				buttons: true,
				dangerMode: true,
			})
	...
	$(document).ready(function() {
		...
		$('#form-add').validate({
			...
			submitHandler: function(form) {
				...
				$.ajax({
					type: "POST",
					url: api + "service/foto_produk/create",
					data: formData,
					async: false,
					processData: false,
					contentType: false,
		...
		$('#form-view').validate({
			...
			submitHandler: function(form) {
				...
				$.ajax({
					type: "POST",
					url: api + "service/foto_produk/update",
					data: formData,
					async: false,
					processData: false,
					contentType: false,
